...
- After the user signs in, a stale tab can still hold the pre-auth sign-in URL, so an auth probe that inspects "the" tab reports unauthenticated even though the profile now has a session. Open a fresh tab through `/json/new?<encoded-url>` and re-verify before declaring the session unusable or relaunching anything.
