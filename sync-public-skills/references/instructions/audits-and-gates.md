...
`copilot-skills/`（`.copilot` 由来ミラー）をpublicへ出す前にskill単位で監査し、結果をdistribution configの `publicCopilotSkills` / `deniedCopilotSkills`へ保存する。`-IncludeCopilotSkills` はallowlist済みSkillだけを公開し、未分類Skillが1件でもあれば停止する。
...
- **④Office/PPTX asset internals**: `.pptx` など Office asset を含む skill は、visible text だけでなく ZIP 内部の `docProps/custom.xml` / `docProps/core.xml` / slides / notes / customXml を scan する。MIP ラベル、個人メール、author 名、tenant ID、ローカルパスが残る場合は public 同期前に削除・中立化し、できなければ除外する
- 既定ブラックリスト例: `docx,pptx,xlsx,expense-report,loop,excalidraw`（①②）＋ `.github/skills` と重複する skill（②）。新規 skill が増えたら上 4 観点で再判定してリストを更新する
- `Sync-AndPush.ps1` は `skill-distribution.json` のdenied / deniedCopilot / internal集合を引数に関わらず除外する。現在値はconfigだけで管理し、このreferenceへ複製しない
