---
name: d365-expense-sorter
description: "D365 / 出張経費専用。Microsoft Dynamics 365 Expense 申請向けに、出張レシートを OCR（Surya）してプロジェクト単位で自動仕分け・リネーム・集計する。複数枚 / PDF / 動画 / 国別 / カード別 / D365 カテゴリマッピング対応。個人事業・確定申告の領収書整理は receipt-ocr を使う。Use when: D365 経費仕分け, 出張経費バッチ, レシート仕分け, expense sorting, Microsoft 出張領収書整理."
...
Microsoft Dynamics 365 Expense 向けの出張経費レシート自動仕分け・リネーム・集計ツール。OCR は Surya を使う。
