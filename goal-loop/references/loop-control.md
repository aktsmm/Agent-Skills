...
4. それでも 4 回 replan して進まなければ HITL。
...
2. **pilot**: 最小スコープ（1 ファイル / 1 ケース / dry-run）でだけ変更する。
3. **verify**: その pilot に Phase 1 の `verify:` を実行し exit code を取る。
4. **判定**:
...
2. **異なる approach を 4 つ以上**試したか？（attempt 教訓 table で確認。同じ手の反復は blocker でない）
...
| backup 不能の不可逆操作              | **HITL**                      | 実行可（rollback 不能を明記しログ）    |
...
- 超自律でも **秘密情報の露出・個人情報の公開・未許可の外部公開は対象外**（別途明示許可）。must NOT と達成の偽装禁止も維持。
