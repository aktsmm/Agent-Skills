...
このスキルを起動した **あなた（現在の agent）が orchestrator** になる。orchestrator は実作業の担当者ではない。
調査・実装・検証は worker / verifier subagent に委譲し、自分は criteria、ledger、evidence reconciliation、次の一手の決定に集中する。
...
- **委譲は明示的に**。"委譲する" と言って自分でやらない。MUST 現在環境で利用可能な subagent tool
  （VS Code では `#tool:agent`、対応環境では `runSubagent`）で worker / verifier を起動する。
- **orchestrator は context を抱え込まない**。full artifact / full log / full transcript を読み込まず、worker から返る
  delta evidence と ledger だけで判断する。広い読解・編集・テスト実行は worker / verifier の仕事。
...
- **粘り強くやり遂げる**。失敗しても reflection → 別 approach → replan で進む。止まるのは「達成」か
  「本物のブロッカー / resource stop」か「Autonomy Mode で認められていない操作」のときだけ。
- **粘り強さを明示する**。Phase 1 で Persistence Profile（Standard / Persistent / Exhaustive）を決め、
  その retry budget 内では HITL 前に別 approach と replan を優先する。
...
| backup を取れない不可逆操作<br/>（本番 deploy、force push、課金/送信 等） | HITL                       | **実行可**（許可済み範囲のみ。rollback 不能を明記しログ） |
...
- **安全弁は Autonomy Mode より優先**する。超自律でも **秘密情報の露出・個人情報の公開・未許可の外部公開は対象外**
  ...
  ただし保留項目が must AC または primary verifier に必要なら、全体を complete とせず HITL / partial handoff にする。
  保留した項目は ledger に記録し、**Phase 7 で Next Action / ハンドオフとしてレポート**する。
- 超自律で criteria を緩和・再定義した場合、完了報告では **original criteria** と **revised criteria** の達成状態を
  分けて示す。改定後 PASS を、元 criteria PASS として扱ってはいけない。
...
**Roles（生成と判定の分離が本質）**: orchestrator は state を所有するが、**自分の成果に最終合格判定を出さない**。
evaluator（毎ループの項目突合）と final reviewer（最後に 1 回の独立横断レビュー）は判定ロールで、規模に応じて
別 subagent に分離する。mutation する worker と判定する reviewer は必ず別人格にする。役割を増やすときも
「読み取り/実装（worker）」と「評価/レビュー（evaluator・reviewer）」の 2 系統に寄せ、無秩序に増やさない。
小規模で final reviewer を省略する場合でも、自己レビューは補助 checklist に限る。最終 PASS は外部 evidence と
frozen criteria の照合でだけ出す。
...
10. **Persistence Profile / Stop 条件**: Standard / Persistent / Exhaustive、max iteration、コスト/時間の上限、confidence 閾値。
    **起動時に明示されていなければここで聞く**。回答が無い場合は既定の Persistent として ledger に固定する。

合意後、criteria を **freeze** する。Normal ではループ中に criteria を自分で緩めてはいけない。
変更が要るときは Phase 6 の HITL（ユーザー再合意）へ。超自律では Phase 6 の Escalation に従い、
`criteria_change` として記録+通知した場合だけ再定義できる。silent mutation と検証弱体化は禁止。

Phase 1 はユーザーとの合意フェーズであり、深い調査フェーズではない。名前付きファイル・repo・既存成果物・
再現手順・関連 instructions は、criteria 合意に必要な最小限だけ確認する。広い読解が必要なら read-only worker に委譲する。
...
### Scope Terminus 確認（手戻り防止・1 問だけ）

ゴールの**終点が幅を持つ**とき（後から「そこまでやって」と言われ手戻りが出るパターン）は、実行前に
**選択式の確認を 1 回だけ**挟む。Autonomy Mode と同じ 1 問に束ねて往復を最小化する。些末タスク
（Effort Scaling の「些末」）は免除。次のいずれかに該当したら確認する:

- **反映の深さが曖昧**: `全部` / `反映` / `対応` / `デプロイ` などで、ソース修正だけか、ビルド・配布・
  **ライブ反映**までかが一意でない（例: テキスト/設定の修正だけか、ビルド・配布してライブに反映するまでか）。
- **表記・命名の揺れ**: タイトル / ラベル / 用語にスペース有無・大小・言い回しの選択肢がある
  （例: ラベルのスペース有無・大小・語順の揺れ）。確定表記をそのまま引用して聞く。
- **適用範囲の境界**: 対象が「アプリのみ / docs のみ / 両方」「履歴記録や生成物まで含むか」で割れる。

確認は「A: ここまで / B: ここまで」の二択以上を提示し、推奨案に既定マークを付ける。回答が来たら
前提として Task Ledger に固定し、以後は再確認しない。
...
1. **pilot**: worker が最小スコープ（1 ファイル / 1 ケース / dry-run）でだけ変更を試す。
2. **外部検証**: worker / verifier がその pilot に `verify:` を実行し、exit code と実出力を orchestrator へ返す。
...
worker は **MUST 現在環境で利用可能な subagent tool** で起動する。read-only 調査は `Explore` 等を並列で使ってよい。
subagent tool が利用不可なら、黙って main context で worker 作業を代行しない。小規模タスクへ降格するか、
degraded mode としてユーザーに確認し、ledger に記録する。
...
worker / verifier が、成果物に対して Phase 1 で決めた `verify:` を実行する（test / lint / build / grep / schema 等）。
**exit code と実出力を取得**して orchestrator へ返す。orchestrator はこの外部 signal を評価の一次情報にする。

- **Primary verifier** は、ユーザーの実際の成功面に最も近い検証にする。失敗と成功を分け、次の修正を選べる
  evidence を返す必要がある。非コード目的（調査 / 執筆 / データ整理 等）では、最終成果物が**入手・閲覧・提出できる状態**
  （公開された記事 / 開ける提出物 / 検索可能なデータ）を primary にする。
...
- 自動検証できるものは worker / verifier が必ずコマンドで検証する。推測で PASS としない。
...
- **生成と評価のロールを分ける**。小規模・read-only・低リスクを除き、evaluator を別 subagent（別 system prompt）として起動する。
...
**evaluator と final reviewer の使い分け**:

- **evaluator** は毎ループ走る項目突合の判定機（rubric で各基準を PASS/FAIL）。小規模・read-only・低リスクを除き、
  中規模以上または worker が mutation した成果物では別 subagent 必須。
- **final reviewer** は完了直前に 1 回だけ走る独立横断レビュー（Phase 7 の final quality gate を実行する人格）。
  「criteria を全項目、独立に再検証する」「invariants が壊れていない」を別 system prompt で見る。
- **orchestrator は自分の成果に最終 PASS を出さない**。小規模は final reviewer を省略できるが、自己レビューは補助に限る。
  **大規模・不可逆は final reviewer を別 subagent として MUST 起動**し、その evidence をもって完了する。
...
flowchart TD
  ...
  A -- Yes --> FG[Phase7 final gate]
  FG -- clean --> DONE[stop-state 出力して完了]
  FG -- non-clean --> RB[review_blocked として継続]
...
- **収束**: 全 must PASS かつ confidence ≥ 閾値 → Phase 7 final gate。gate が不要または clean の場合のみ完了。
- **stall**: `stall_counter` が profile の `stall→replan` 閾値に到達 → replan。
- **oscillation**: 同一の `(action, target)` の繰り返し検知 → replan か HITL。
- **diminishing returns**: profile で定めた連続回数だけ改善幅が ε 未満 → HITL。
- **budget**: profile の max iteration / コスト・時間上限 超過 → HITL。
...
4. **resource / confidence stop**: diminishing returns / budget 超過 / confidence 低迷。これは本物のブロッカーとは別分類。
...
- 超自律で criteria を改定した場合は、**original criteria** と **revised criteria** を分け、元基準が未達なら未達と明記する。
...
- 大きなコード変更・設計変更では、最終報告前に final quality gate を通す: targeted verify → cleanup → re-verify →
  invariant audit → independent review。independent review は **final reviewer（別 subagent）が担当**し、
  orchestrator は自分の成果に最終 PASS を出さない。非 clean なら `review_blocked` とし、blocker 解消サブゴールを追加して続行する。
...
- "委譲する" と書いて自分で全部やる。
- orchestrator が worker の代わりに広い調査・実装・pilot・primary verifier 実行を抱え込む。
- worker に checkpoint / complete / blocked 判定を任せる。
- 大規模・不可逆ゴールで orchestrator が自分の成果に最終 PASS を出す（final reviewer を起動せず self-enhancement bias を放置する）。
