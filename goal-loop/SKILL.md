...
1. **Goal**: 1 文で達成状態を書く。
2. **受入基準（Acceptance Criteria）**: 各基準に **`verify:` 検証方法（コマンド / 観測手順）** を必ず添える。
   検証できない基準は、検証可能な形に書き直す。
3. **非ゴール（Out of Scope）**: やらないことを明記する。
4. **must NOT（抜け道の封鎖）**: criteria を形式上満たす不正な近道を禁止する
   （例: test を消す / 出力を切り詰める / TODO で埋める / ハードコードで通す）。
5. **制約**: 触ってよい範囲、使ってよい tool、破壊的操作の可否。
6. **Autonomy Mode**: Normal か 超自律（AUTO / FULL / ALL）か。**起動時に明示されていなければここで聞く**。
7. **Stop 条件**: max iteration、コスト/時間の上限、confidence 閾値。
