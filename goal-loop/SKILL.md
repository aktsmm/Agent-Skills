...
| 小（数ステップ・低リスク） | 軽量                       | 1–2 直列                       | 外部検証 1 回      | orchestrator 自己レビュー可 | 暗黙でよい         | 任意           |
...
- **evaluator** は毎ループ走る項目突合の判定機（rubric で各基準を PASS/FAIL）。中規模以上は別 subagent 推奨。
...
- **orchestrator は自分の成果に最終 PASS を出さない**。小規模は orchestrator 自己レビューで可だが、
