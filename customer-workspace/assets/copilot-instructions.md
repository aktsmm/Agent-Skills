...
新規の受領原文がある場合は「受領メモの原文優先」を先に適用する。それ以外は、ユーザーの入力が明確なコマンドや質問でない場合に以下の順で判定し自動処理します。

### 受領メモの原文優先

お客様から打ち合わせメモ・議事録・メール本文などの原文が新規に提示された場合は、同じメッセージに質問があっても回答・要約・宿題抽出より先に取り込む。

1. 会議日が分かる場合は `_received/mtg-YYYY-MM-DD-{topic}/`、不明な場合は `_received/incoming/` に、語句・記号・段落構造を変えない原文ファイルとして保存する。
2. `attachments.md` に出どころ・機密区分・関連議事録を記録し、既存または新規の議事録から原文へリンクする。
3. 原文の保存とリンク付けが完了してから、原文を根拠に回答・変換・タスク抽出を行う。既存の要約済み議事録だけを根拠にしない。
...
- 契約情報（期間、金額、形態の変更）
- 組織情報（担当者異動、新規連絡先、会議で共有された組織体制・ベンダー体制）
- 技術スタック（新規導入、廃止）

## 案件ポートフォリオと更新情報

このワークスペース全体を親プロジェクトとし、`workstreams/` 配下の各フォルダを案件として管理する。全件の一覧は `workstreams/README.md`、案件ごとの現在状況、現在のボール、アクション、履歴の正本は各 `workstreams/{workstream-id}/README.md` とする。

- Teams、メール、会議メモ、貼り付けられた更新情報は、保存後に案件名、判定キーワード、現在の範囲で既存案件と照合する。
- 既存案件が 1 件に明確に一致した場合は、短い入力の原文を `_inbox/` に残し、案件 README の状態、現在のボール、履歴を更新する。詳細が必要な場合だけ `updates/` に日付付き要約を作成する。
- 会議メモは `meeting-notes/` を原本とし、案件 README には関連ファイルへのリンクと案件状態の変更だけを記録する。議事録全文を複製しない。
- 新規、複数該当、または低確信の場合は `workstreams/_candidates.md` に候補として記録して確認する。この段階では案件フォルダを作成しない。
- 専用の証跡・長期履歴が必要になった案件だけを `pj_{topic}/` に切り出す。
...
- `triage-workstream-update.prompt.md` - 案件更新の振り分け
...
- 同一トピックの追加質問・再回答は、初回の質問と回答を残し、所有する成果物または案件履歴へ時系列で追記する。月次 `_questions/` 台帳では当月側に追記して元質問の日付またはリンクを示し、過去月の記録を上書きしない。
...
`meeting-notes/` と `next-actions/` はどちらも時間軸の整理なので、複数回の定例をまたいで続くテーマの置き場がない。放置すると 1 本の議事録に追記が積み上がり、現状と経緯が読み分けられなくなる。

- **切り出す条件**: 複数回の定例をまたぐこと（必須）。加えて「関連ファイルが既に 3 つ以上ある」「専用の証跡が溜まる（ログ、ベンダー回答、外部ケース ID）」「担当が定例の常連メンバーと違う」のいずれかを満たすこと。必須条件だけなら `next-actions/ongoing/` に置く。
- **命名**: `pj_{topic}/`（`{topic}` は kebab-case）。日付プレフィックスは付けない。外部ケース ID はフォルダ名に入れず `README.md` に書く。
- **正**: 現在状況と経緯はテーマフォルダ側が持つ。議事録からはリンクするだけで、状況を書き写さない。
- **README**: 冒頭で「いま誰がボールを持っているか」が 1 行で分かること。
- **移行のタイミング**: 同じ議事録に 3 回以上の会議にまたがって追記された、またはそのテーマの節が議事録の残りの節を合計した長さを超えた時点で切り出す。移動前に他ファイルからの参照を検索し、移動後にリンクを直す。
- **クローズ**: 明示的に閉じるまでテーマは open のまま残る。未完アクションが無く、2 定例分の更新も無ければクローズ候補として扱う。閉じるときは `README.md` に結果と終了日を書き、未完アクションを `done` / `dropped` に揃える。フォルダは削除しない。
