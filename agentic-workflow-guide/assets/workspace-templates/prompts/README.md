...
以下のプロンプトだけが `scaffold_workflow.py --full-workspace` で生成先へコピーされます。
...
> **Note**: Git操作・デバッグ・レビュー系の汎用プロンプトは VS Code のグローバル設定に配置することを推奨します。`assets/workspace-templates/prompts/` に他の prompt が存在しても、full workspace scaffold ではコピーしません。
