...
- `.github/instructions/**/*.instructions.md` は再帰的に読み込まれるため、`core/` `dev/` `agents/` `integrations/` などのサブフォルダ配下でも問題ない
- 効くかどうかは「直下かどうか」ではなく `applyTo` パターンで決まる
- そのため、整理するときは直下に平置きするより、責務ごとにサブフォルダへ分ける方を優先してよい
