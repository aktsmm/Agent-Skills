...
4. 変更系検証なら、Activity Log / resource state / learned routes / BGP peer status / 主要メトリクスのどれを何秒間隔で採るか決める
5. 可能なら変更前から polling 用スクリプトを起動し、時刻付きで証跡を保存する
...
4. 変更対象 resource の Activity Log を取れる形にしておく
5. 変更前の健全性と観測開始時刻を保存する
...
4. 変更要求の開始時刻、Accepted/Succeeded 時刻、correlation ID を保存する
...
4. control-plane 完了時刻と、実際の route / health / metric 変化時刻の差を分けて書く
5. `瞬断なしを証明できた` のか、`瞬断は見えなかっただけ` なのかを分けて評価する
