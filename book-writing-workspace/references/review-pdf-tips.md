...
If the manuscript Markdown H1 already contains a literal chapter prefix such as `第7章 ...`,
Re:VIEW may still add its own chapter number in the PDF or EPUB output.
That produces headings like `第4章 第7章 ...` when chapter numbering is also generated by the builder.
...
If you regenerate only a subset of chapters and also regenerate `catalog.yml` for that subset,
