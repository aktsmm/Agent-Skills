...
In Re:VIEW 5.x, when `titlefile` is set in `config.yml` to use a custom titlepage template,
`\review@titlepageauthors` is **not populated** by the builder — it stays empty (`\def\review@titlepageauthors{}`).
The custom titlepage renders without any author name even though `aut:` is correctly set in `config.yml`.

### Solution

Inject the author definition via a `.sty` hook file that is loaded by `review-custom.sty`:

1. Generate `sty/review-titlepage-hook.sty` from the build script or metadata helper:

```latex
\def\review@titlepageauthors{Author Name\\[0.3em] 著}
```

2. Load it in `review-custom.sty`:

```latex
\InputIfFileExists{sty/review-titlepage-hook.sty}{}{}
```

### Font Size Pitfall in Custom Titlepage

Do not wrap `\review@titlepageauthors` in a `tabular` environment with `p{\textwidth}`.
The paragraph-column mode suppresses `\fontsize` directives, making the author name
appear much smaller than intended.

Instead, output the text directly inside a font-size group:

```latex
{\fontsize{28pt}{36pt}\selectfont\bfseries
\review@titlepageauthors\par}
```

### Why This Matters

The empty-author bug is silent — the PDF builds without error, and the titlepage simply
shows no author. The `tabular` suppression is equally subtle: `\fontsize` appears to be
set correctly in the `.tex` source, but the rendered output stays at the column's default size.

## Dual PDF Build: Print and Ebook
...
A single build script can produce both variants by maintaining two config files:

1. `config.yml` — primary config with `media=print`
2. `config-ebook.yml` — generated by replacing `media=print` with `media=ebook` and stripping `openright`

```python
# Generate ebook config from print config
opts = config_data["texdocumentclass"][1]
opts = opts.replace("media=print", "media=ebook")
opts = opts.replace("openright", "").strip().rstrip(",")
config_data["texdocumentclass"][1] = opts
```

### CLI Flags

```
--print-only    Build only the print PDF
--ebook-only    Build only the ebook PDF
(no flag)       Build both
```
...
and ebook for digital stores or review copies. Building both from a single source
eliminates drift between the two outputs.
