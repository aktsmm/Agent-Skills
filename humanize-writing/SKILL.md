...
- pivot 構文: `A ではなく B` `A というより B` で観察の代わりに対比を置く。節の冒頭を `A だけでは決めません` と否定から始めるのも同類。
- 例の偏り: ツールの使い分け記事で例がコード修正・テスト・ログ調査だけに寄る。要約・調査・文書作成など読者の日常業務も並べ、筆者が試していない例は「〜なら〜で足ります」の勧め方で書く。
...
- **副詞偏愛 cluster**: 1 記事内で `個人的に` `本当に` `ちゃんと` `ものすごく` など同じ副詞が **3 回以上**出たら、個々は High でなくても累積で AI 臭になる。箇所リストを作って 3 件以上は減らす。
