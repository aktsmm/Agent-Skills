...
> 🔴 **重要**: Supported Regions リストに**片方だけ**記載されている場合、記載されていない方は**未対応**と判定する。このルールは**リストが存在する場合のみ**適用し、リージョン節自体がないページには適用しない（→ 優先度 4a）。
...
→ **両方なし** → 「日本リージョン未対応」
⚠️ リストに片方だけある場合、他方は未対応。ただしリージョン節自体がないページは「制限なし」→ 優先度 4a
⚠️ 絵文字（✅❌）を含む形式は使用しない

優先度 4a: Preview / GA で機能ドキュメントはあるがリージョン制限の記載なし
→ 「Japan East / West 対応」（親サービスの日本提供で判定、サインアップ必須の場合あり）
→ evidence に『【判定レベル: リージョン限定記載なし】』と親サービス名を記録

優先度 4b: Preview 機能でリージョン限定の記載あり
→ 記載されたリージョンのみ対応

優先度 4c: 機能ドキュメント自体が存在しない（Azure Updates 原文のみ）
→ 「日本リージョン未対応」と断定しない。evidence に『【判定レベル: 根拠未取得】』を記録し、表示方法を依頼者に確認

優先度 5: GA 機能でリージョン制限の記載なし
→ 「グローバル」（全リージョン対応と推定）

優先度 6: 情報不足
→ 4c へエスカレーション。無記載だけを根拠に「日本リージョン未対応」へ倒さない
...
| 記載なし = 対応と誤認                      | -                               | デフォルトで対応扱い   | 提供リージョン一覧に片方だけある場合、他方は未対応              |
| リージョン節自体がないことを未対応と誤認  | Azure Firewall IPv6 他 10 件    | 日本リージョン未対応   | Japan East / West 対応（制限記載なし = 制限なし）     |
