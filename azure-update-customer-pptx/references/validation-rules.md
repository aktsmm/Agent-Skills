...
41. When you add a gate, prove it executes on the **standard pipeline path**, not only when invoked directly. A gate wired to an optional parameter that the orchestrator never passes skips silently on every real run while passing every isolated test. Read the gate's own line out of the pipeline's verify log before believing it. Treat missing script, unresolved folder, interpreter not found, and launch failure as FAIL, never as a skip — a fail-open gate is indistinguishable from no gate.
