...
- Findings can be inflated the same way, so verify before you act. A critic scanning by pattern reports matches that are correct in form but harmless in context, and it can attach a count that never held. Re-derive any count or list with your own deterministic check, act on the verified number, and say which findings you dropped and why.
