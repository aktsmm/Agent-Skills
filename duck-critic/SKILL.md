...
- For UI/design checkpoints, distinguish appearance preferences from defects that obstruct the user's task; require current actual-render evidence and inspect it rather than inferring visual acceptance from build success or the producer's verdict.
...
- Apply the same suspicion to **your own** research. A number you got from an exploration subagent, a summary, or a memory of an earlier session is unverified until you measure it, and once it is in the packet the critic argues against a false baseline. Measure the counts you are about to assert, and when the critic disputes one, re-measure rather than picking a side — both numbers can be wrong.
