...
- When the critic can read the repository, name the files instead of pasting them, and bound the list. An unbounded "review the workspace" produces findings about code the round was never about. A critic reading files also states things about implementation state it did not actually open, so require file and line for every such claim and open them yourself before acting. Its file access is also scoped to its own harness workspace, so a target living outside that workspace — another repo, a mirrored copy, an edit not yet written where the critic can see it — silently resolves to whatever same-named file the workspace does hold. The critic then reports "that section does not exist", with a file link, correctly, about the wrong copy. When the target is outside the workspace, quote the changed regions in the packet and say the paths are context only.
- When the question is "what did I miss", the inventory **is** the packet: list the source's full feature set, list your own adopt / reject mapping, and ask the critic to name what appears in **neither** column. Without the mapping it re-lists the source back at you; without the inventory it can only judge what you already noticed.
