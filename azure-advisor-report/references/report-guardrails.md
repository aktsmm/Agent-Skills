...
- 設定された全環境の収集statusが成功し、各環境にCost 1件と全Advisorカテゴリのrawが存在すること
- データ取得日と通貨コードがMarkdown/PPTXに明記され、summaryの値と一致すること
- PPTXのスライド数、PDFページ数、speaker notes件数が一致すること
- PDFが非暗号化要件を満たし、正規PPTXをローカル一時コピーからPDF化した前後でSHA-256が一致すること
- 顧客向け本文に認証コマンド、ローカル絶対パス、内部URL、内部ツール名が残っていないこと
- gate結果をmachine-readable JSONへ保存し、任意の1チェックでも未実行または失敗なら配布完了にしない
