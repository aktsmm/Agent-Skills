...
- **資源名そのものは無条件にマスクしない**。ラボ用に作った短命リソース名 (`stmockrun260525a` など) は本文で説明済みなら残してよい。テナント / アカウント / 個人特定に直結する識別子だけを対象にする
...
逆に、汎用の lab 用に作った storage account 名や resource 名は、本文に明記して説明している前提なら残してよい。
