...
| Copilot Scheduler (VS Code extension) | VS Code-local scheduled prompt execution with cron expressions, task scope, prompt templates, and run history. Per-task model selection is version-dependent; verify the task schema | Verify extension settings, task-level limits, prompt template paths, workspace scope, jitter, max daily executions, how the run resolves its model, an OS-level pre-stage job that opens the editor, and Copilot Chat approval behavior before unattended runs. Record adapter version with the observed behavior. |
