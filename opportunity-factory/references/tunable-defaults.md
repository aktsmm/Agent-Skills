...
| Review repair workflow rounds                                                                                                                      | 10（下限 3 / 上限 20）。既存 persistence の残り iteration と小さい方を上限にする                                                       | unresolved finding、replan率        | workflow-review が範囲内で調整                           |
| Independence block limit                                                                                                                           | 3（下限 1 / 上限 5）。同一 parent task の独立critic不在を `parked-independence` へ上げる回数                                          | adapter/model outage、approval数    | workflow-review が範囲内で調整                          |
...
| Throughput attempt 目標 / lane cap                                                                                                                 | workspace ごと                                                                                                                         | Gate 通過率の推移                   | workflow-review 調整 (`workflow.md` `## Throughput KPI`) |
| Publish 見込み / Gate 通過率目標                                                                                                                   | workspace ごと (publish は soft forecast)                                                                                              | 通過率が安定したときだけ attempt 増 | workflow-review propose → user 承認                      |
...
| Critic 3 Layer の存在 + Independence 契約 (Layer 3 は別 model family 必須 / fail-closed)                                                           | (**hard rule**)                                                                                                                        | —                                   | 変更禁止                                                 |
| Review repair contract（substantive repair 後の独立 re-review / durable round record / repair の自己認証禁止）                                     | (**hard rule**)                                                                                                                        | —                                   | 変更禁止                                                 |
...
2. `references/rubber-duck-review.md` の "Layer 3 Blocking Critic" 節に 5 gate が全部あるか
...
6. `references/rubber-duck-review.md` に "Repair -> Re-review Contract" と `blocked-independence` があり、`runtime.limits.reviewRepairRounds` が 3〜20、`independenceBlockLimit` が 1〜5 を維持しているか
...
- `references/rubber-duck-review.md`: Critic Layer / Layer 3 Blocking Critic SSOT
