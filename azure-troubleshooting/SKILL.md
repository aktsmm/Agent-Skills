...
サポートや他ベンダーからの技術回答をそのまま顧客へ渡すと、被疑箇所が並列に並んだだけの文書になり、顧客はどこから着手するか判断できない。次の形へ読み替えてから渡す。

- 項目ごとに「どのホストで、誰が実施するか」を先に確定する。回答本文に書かれていないことが多く、ここが欠けると認識違いのまま作業が始まる。
- 回答内で「可能性は低い」と評価済みの項目は明示的に降格する。並列表記のままだと全件調査に見え、顧客の作業量が実態より膨らむ。
- 「いま実施できること」と「再現時に採取するもの」を分ける。断続事象では、再現時採取の準備を先に終わらせないと証跡そのものが取れない。
- 元回答のセクションと作業項目の対応表を付け、顧客が原文と並べて読めるようにする。
- 多段の依存関係（CNAME チェーン、経路、依存サービス）は文章ではなく図で示し、どの段で落ちているかを 1 目で分かるようにする。
- 元回答に無い推測を混ぜない。こちらの解釈は解釈として区別して書く。
